        ...
        <div class="row justify-content-center">
            <div class="col-md-6">
                <!-- <div class="card"> -->
                    ...
                    <div class="card-body">
                           <form action="{{route('update.announcement',$basicId)}}" method="post" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                            <div class="form-group">
                                <label for="title" class="text-capitalize">Title Announcement*</label>
                                <input type="text" class="form-control" id="title" name="title" value="{{$announce->title}}" placeholder="Enter title">
                                @error('title')
                            ...
                            <div class="form-group">
                                ...
                                <select name="status" id="status" class="form-control" required>
                                    ...
                                    <option value="public" {{ $announce->status === 'public' ? 'selected' : '' }}>Public</option>
                                    <option value="private" {{ $announce->status === 'private' ? 'selected' : '' }}>Private</option>
                                </select>
                                @error('status')
                                <div class="text-danger">{{ $message }}</div>
                                @enderror
                            ...
                            <div class="form-group" id="divisions-group" style="{{ old('status') === 'private' ? 'display: block;' : 'display: none;' }}">
                                ...
                                <select name="divisions[]" id="divisions" class="form-control" multiple>
                                    ...
                                    {{ $division_ids->contains($divs->divisi_id) ? 'selected' : '' }}>
                                        {{ $divs->divisi_name }} 
                                    </option>
                                    @endforeach
                                </select> 
                            ...
                            <div class="form-group">
                                ...
                                <textarea class="form-control" name="description" id="description" cols="10" rows="3">{{$announce->description}}</textarea>
                                @error('description')
                                <div class="text-danger">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="form-group">
                                <label for="file" class="text-capitalize font-weight-bold">Upload File (PDF)*</label><br>
                                <small class="text-capitalize font-weight-bold">Name File OLD : {{$announce->file_name}}</small>
